class UserPasswordController < ApplicationController
  ...
  <% unless options[:skip_layouts] -%>
    layout "authentication", :only => [ :edit, :update ]
  <% end -%>
  ...
  def reset_password
    respond_to do |format|
      if @user = User.find_by_login_and_reset_password_token(params[:login], params[:token])
        format.html
      else
  ...
  def update_password
    @user = User.find_by_login_and_reset_password_token(params[:user][:login], params[:user][:reset_password_token])
